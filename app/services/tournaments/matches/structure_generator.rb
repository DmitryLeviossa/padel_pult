  ...
  module Matches
    class StructureGenerator
      ...
      def generate_olympic
        ...
        (n / 2).times do |i|
          @tournament.matches.create!(round_number: 1, position: i + 1, status: :pending)
        end

        (2..total_rounds).each do |round|
          (n / (2**round)).times do |i|
            @tournament.matches.create!(round_number: round, position: i + 1, status: :pending)
          end
        end

        if total_rounds > 1
          @tournament.matches.create!(round_number: total_rounds, position: 2, status: :pending)
        end
      ...
      def create_loser_bracket_structure(main_n)
        ...
        (loser_n / 2).times do |i|
          @tournament.matches.create!(round_number: 1, position: i + 1, status: :pending, stage: :loser_bracket)
        end

        (2..loser_total_rounds).each do |round|
          (loser_n / (2**round)).times do |i|
            @tournament.matches.create!(round_number: round, position: i + 1, status: :pending, stage: :loser_bracket)
          end
        end

        if loser_total_rounds > 1
          @tournament.matches.create!(round_number: loser_total_rounds, position: 2, status: :pending, stage: :loser_bracket)
        end
      end
      ...
      def generate_round_robin
        ...
        (n - 1).times do |round_idx|
          (n / 2).times do |i|
            @tournament.matches.create!(round_number: round_idx + 1, position: i + 1, status: :pending)
          end
        end
      end
      ...
      def pre_create_bracket(stage, size)
        ...
        (1..total_rounds).each do |round|
          (n / (2**round)).times do |i|
            @tournament.matches.create!(
              round_number: round, position: i + 1,
              status: :pending, stage: stage, group_number: 0
            )
          end
        end
        ...
        @tournament.matches.create!(
          round_number: total_rounds, position: 2,
          status: :pending, stage: stage, group_number: 0
        )
      end
